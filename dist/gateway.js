...
import { createAssistant, executeCommand, parseCommand } from './index.js';
import { setPeerBase } from './peer-require.js';
import { createProactiveCoordinator } from './proactive.js';
...
export function clearGroupChatState(sessionKey) {
 ...
 */
export function purgeIdleGroups() {
...
export function resolveGroupChatConfig(config) {
...
export function resolveStreamingConfig(config) {
...
export function buildGroupPrompt(history, senderName, userText, injectPass, groupKey, _dir, 
...
injectContinue = false) {
    ...
    if (injectPass) {
        ...
        if (othersAddressed && othersAddressed.length > 0) {
            ...
            parts.push(`[System: This message is directed at ${names}, not you. ` +
                'Only respond if you have something important to add or correct. ' +
                'If you have nothing essential to contribute, respond with exactly: [PASS]]');
        }
        else {
    ...
    if (peers && peers.length > 0) {
        ...
        if (!injectPass) {
            parts.push('[System: Other bots listed above are your peers. ' +
...
export function requireFields(type, config, fields) {
 ...
 */
export function parseMentions(text, memberCache) {
    const mentions = [];
...
async function createChannelAdapter(type, channelConfig, dir) {
    switch (type) {
        ...
        default: {
            ...
            const resolvedPath = adapterPath.startsWith('.') || adapterPath.startsWith('/') ? resolve(dir, adapterPath) : adapterPath;
            let mod;
            try {
                mod = await import(resolvedPath);
            }
            catch (e) {
                throw new Error(`Failed to load custom adapter "${adapterPath}": ${e.message}`);
            }
 ...
 */
export async function handleMessage(msg, config, assistant, adapter, channelType, verbose, dir, metrics, cronCtx, 
/** Fleet peers for multi-bot awareness. */
peers) {
    const userText = msg.chatType === 'group' ? stripMention(msg.text) : msg.text;
    if (!userText && (!msg.images || msg.images.length === 0) && (!msg.files || msg.files.length === 0))
        return;
    ...
    if (msg.chatType === 'group') {
        ...
        if (gc.groupPolicy === 'mention-only' && !mentioned)
            return;
    ...
    const sendStatusUpdate = async (text) => {
        if (!text.trim())
            return;
    ...
    const runOneRound = async (promptText, mayRelay) => {
        ...
        if (effectiveMode === 'streaming') {
            ...
            const flush = async (reason) => {
                ...
                chunkIdx++;
                console.log(`[stream-debug] chunk #${chunkIdx} (${reason}): "${buffer.trim().slice(0, 80)}…" [${buffer.length} chars]`);
 ...
 */
export function channelMsgToInbox(msg, sessionKey, fullText) {
    return {
        sessionKey,
        message: fullText,
 ...
 */
export function startInboxConsumer(inbox, processEntry, verbose) {
    let stopped = false;
    let timer;
    const loop = async () => {
        while (!stopped) {
...
export async function startGateway(opts) {
    ...
    assistant.resetSession = async (sessionKey) => {
    ...
    if (channels) {
        for (const [type, channelConfig] of Object.entries(channels)) {
            if (!channelConfig)
                continue;
            try {
                const adapter = await createChannelAdapter(type, channelConfig, dir);
                ...
                channelStatuses.push({ type, status: 'failed', error: e.message });
            }
        }
    }
    ...
    if (inboxStore) {
        ...
        inboxConsumer = startInboxConsumer(inboxStore, async (entry) => {
        ...
        const compactTimer = setInterval(async () => {
            ...
            if (removed > 0)
                log(verbose, `[inbox] Compacted ${removed} old entries`);
        }, 6 * 60 * 60 * 1000);
        compactTimer.unref();
    }
    ...
    if (inboxStore && config.historyFetch?.enabled) {
        ...
        const startupTimer = setTimeout(async () => {
            try {
                ...
                console.error('[history-fetch] Startup fetch failed:', e.message);
            }
        }, startupDelayMs);
    ...
    if (config.tasks && config.tasks.length > 0) {
        try {
            ...
            console.error('[scheduler] Failed to initialize tasks:', e.message);
        }
    }
