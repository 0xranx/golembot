...
- Only text messages are received; the bot can send images (PNG, JPG, GIF; ≤10MB) and files (≤20MB, min 5 bytes) as separate media messages via `[SEND_IMAGE: <path>]` / `[SEND_FILE: <path>]` markers, delivered right before the completion marker. Agents save to `temp_file/` (workspace-relative) by default; `/tmp` is also accepted.
